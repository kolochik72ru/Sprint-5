...
func (w Walking) Calories() float64 {
	...
	return ((CaloriesWeightMultiplier*w.Weight + (poweredMeanSpeedInMs/(w.Height/100))*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours() * MinInHours)
}
...
func ReadData(training CaloriesCalculator) string {
	calories := training.Calories() // получаем количество затраченных калорий
	info := training.TrainingInfo() //получаем информацию о тренировке
	info.Calories = calories        //добавляем полученные калории в структуру с информацией о тренировке
